     ...
     try {
		 ...
		 if(oUsuarioResponse.UsuarioRol){
			oUsuarioResponse.UsuarioRol.forEach(function(e){ 
				...
				sRolAsignado = sRolAsignado + "|"+ e.CodRol ;

				
			});
			//sRolAsignado = sRolAsignado.slice(1, -1);
		 }
		 oUsuario.UsuarioRol	= sRolAsignado.slice(1); 
		 var sMaestroAsignado = "";
		 if(oUsuarioResponse.UsuarioMaestra){
			oUsuarioResponse.UsuarioMaestra.forEach(function(e){ 
				sMaestroAsignado = sMaestroAsignado + "|"+ e.CodMaestra ;
			});
			//sMaestroAsignado = sMaestroAsignado.slice(1, -1);
		 }
		 oUsuario.UsuarioMaestra = sMaestroAsignado.slice(1); 

		 //Concatenamos los accesos a la empresa
		 var aEmpresaAsignado = [];
		 if(oUsuarioResponse.UsuarioEmpresa){
			oUsuarioResponse.UsuarioEmpresa.forEach(function(e){  
				aEmpresaAsignado.push({
		 ...
		 oUsuario.UsuarioEmpresa = aEmpresaAsignado; 

		 var appPermitidos = [];
		 var sPermisosxCodigo = "";     
		 if(aRol !== undefined 
				&& aRol.length > 0){ 
					var oFiltroApp = {};
		 			oFiltroApp.aCodRol = []; 
					 aRol.forEach(function(e){
					...
					if(consultarAplicacionRolResponse.iCode !== 1){
					 ...
					 appPermitidos.forEach(function(e){ 
						sPermisosxCodigo = sPermisosxCodigo + "|"+ e.Aplicacion.Codigo ;
				  	 }); 
					   sPermisosxCodigo =  sPermisosxCodigo.slice(1);
		 }
		...
		// console.log(oUsuario.toJSON());
		 var oNewusuario = {};
		 oNewusuario.Datos   = oUsuario;
		 oNewusuario.Datos.UsuarioPermisos = sPermisosxCodigo ;
