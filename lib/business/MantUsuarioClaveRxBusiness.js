...
exports.generarToken = async (req, res) => { 
	...
	try {
		oRequest		 = utils.customRequest(req);  
		var sTiempoExpiracion = config.tiempoSinExpiracion;
		if(oRequest.oData.sTiempoExpiracion){
			sTiempoExpiracion = oRequest.oData.sTiempoExpiracion;
		}
		//Validamos el acceso del usuario
		var oUsuario = oRequest.oData;
		//var sToken = jwt.sign( JSON.stringify(oUsuario), config.claveJwt, {expiresIn: '1h'});
		var sToken = jwt.sign( { data: JSON.stringify(oUsuario) }, config.claveJwt, { expiresIn: sTiempoExpiracion });		
		 oResponse.iCode 		= 1; 
		oResponse.sMessage		= 'OK';
