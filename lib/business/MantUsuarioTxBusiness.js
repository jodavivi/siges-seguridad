...
exports.actualizarUsuario = async (req, res) => { 
	...
	try {
		...
		var oEmpresaActual = {};
		oUsuario.UsuarioEmpresa.forEach(function(e){
			if(e.CodEmpresa === oEmpresa.CodEmpresa){
				oEmpresaActual = e;
			}
		}); 
		//Actualizamos estado de la empresa asignada
		var oParamEmpresa = {};
		oParamEmpresa.oAuditRequest  = oRequest.oAuditRequest;
		oParamEmpresa.oData		  = {}; 
		oParamEmpresa.oData.iId     	= oEmpresaActual.Id;  
		oParamEmpresa.oData.iCodEstadoItem = oData.iCodEstadoUsuario;
		oParamEmpresa.oData.sEstadoItem    = oData.sEstadoUsuario; 
		var actualizarUsuarioEmpresaResponse =  await   usuarioEmpresaTxDao.actualizarUsuarioEmpresa(oParamEmpresa);
		if(actualizarUsuarioEmpresaResponse.iCode !== 1){
		   throw new Error(actualizarUsuarioEmpresaResponse.iCode + "||" + actualizarUsuarioEmpresaResponse.sMessage);
	    } 
		...
		//Verificamos si tiene mas de una empresa 
		if(oUsuario.UsuarioEmpresa.length > 1){
			// No cambiamos el estado del usuario
			oTabla.oData.iCodEstadoUsuario = undefined ;
			oTabla.oData.sEstadoUsuario = undefined;
		}
...
exports.eliminarUsuario = async (req, res) => { 
	...
	try {
		...
		oRequest.oData.aItems.forEach(async function(e){
			...
			//Verificamos si ya exista el usuario
			var oFiltroTabla = {};
			oFiltroTabla.iId = parseInt(e, 10);  
			var consultarUsuarioResponse =  await usuarioRxDao.consultarUsuario(oFiltroTabla);
			if(consultarUsuarioResponse.iCode !== 1){
			throw new Error(3 + "||" + "El Usuario: "+oRequest.oData.sUsuario +", no existe.");
			}
			var oUsuarioActual = consultarUsuarioResponse.oData[0]; 

			var oEmpresaActual = {};
			oUsuarioActual.UsuarioEmpresa.forEach(function(x){
				if(x.CodEmpresa === oEmpresa.CodEmpresa){
					oEmpresaActual = x;
				}
			}); 

			//eliminamos la empresa asignada 
			var oParamEmpresa = {};
			oParamEmpresa.oAuditRequest  = oRequest.oAuditRequest;
			oParamEmpresa.oData		 		 = {}; 
			oParamEmpresa.oData.iId     	 = oEmpresaActual.Id;    
			var eliminarUsuarioEmpresaResponse =  await   usuarioEmpresaTxDao.eliminarUsuarioEmpresa(oParamEmpresa);
			if(eliminarUsuarioEmpresaResponse.iCode !== 1){
				throw new Error(eliminarUsuarioEmpresaResponse.iCode + "||" + eliminarUsuarioEmpresaResponse.sMessage);
			} 

			if(oUsuarioActual.UsuarioEmpresa.length === 1){
				// si solo tiene una empresa se elimina el usuario
				var oUsuario = {};
		 ...
		 oRequest.oData.aItems.forEach(async function(e){
			...
			oUsuarioRol.oData.sCodEmpresa = oEmpresa.CodEmpresa;
			const eliminarUsuarioRolResponse = await  usuarioRolTxDao.eliminarUsuarioRol(oUsuarioRol);
			if(eliminarUsuarioRolResponse.iCode !== 1){
				throw new Error(eliminarUsuarioRolResponse.iCode + "||" + eliminarUsuarioRolResponse.sMessage);
			}
